if(command == "incompletemaps"){
  ...
  let keys = mapDatabase

  for(var a = 0; a < keys.length; a++){
    if(database[person.id]["maps"].indexOf(keys[a]) == -1) mapKeys.push(keys[a]);
  }
  ...
  for(map in mapArray){
    ...
    embed.addField(`${type}`, `Not Beaten: ${mapArray[map][1].length}/${count}`);
  ...
  for(var a = 0; a < mapArray.length; a++){
    ...
    let totalpoints = totalPointsType(person.id, mapname, true);
    ...
    for(var b =0 ; b < maplist.length; b++){
      ...
      for(var c = 0; c < maplist[b].length; c++){
        mapstring += `${maplist[b][c][0]}, Points: ${maplist[b][c][1]}\n`;
      }

      //embed.addField(`${mapname}`, `#${rank}\nPoints: ${totalpoints}`)
      //embed.addField(`Total Points-${mapname}`, `${totalpoints}`);
      embed.addField(`${mapname} - Page ${b+1} / ${maplist.length} - Not Beaten Maps`, mapstring);
      embed.setColor(`#FD0061`);
      embedPages.push(embed);
